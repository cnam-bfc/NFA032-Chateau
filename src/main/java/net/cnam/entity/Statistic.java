...
//METHODES
    ...
    public boolean compareAccuracy(Statistic accuracyTest){
        if (this.accuracy == accuracyTest.getAccuracy()) return (Math.random() <= 50);
    ...
    public boolean compareStrength(Statistic strengthTest){
        if (this.strength == strengthTest.getStrength()) return (Math.random() <= 50);
    ...
    public boolean compareSpeed(Statistic speedTest){
        if (this.speed == speedTest.getAccuracy()) return (Math.random() <= 50);
